...
Теперь мы попробуем пообщаться с языковой моделью **gpt-3.5-turbo** через отправку данных на сервер с помощью **POST** запроса. Согласно [официальной документации по исходящим запросам в OpenAI API](https://platform.openai.com/docs/api-reference/making-requests) мы можем сделать это с помощью запроса к адресу:
```
